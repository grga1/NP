// Дадена е класата Applicant (кандидат за евалуација) со име, кредитен резултат (credit score), 
// работен стаж (experience) и дали има криминално досие (criminal record).
// Исто така, даден е интерфејс за евалуација на кандидати Evaluator со еден метод boolean evaluate(Applicant applicant) 
// и енумерација со следните видови на евалуатори:
...
// кој ќе враќа објект од соодветна имплементација на интерфејсот Evaluator за соодветниот тип на евалуација. 
// Ако типот на евалуација не е некој од наведените, методот треба да фрли исклучок од тип InvalidEvaluation.
...
class EvaluatorBuilder {
    public static Evaluator build(Evaluator.TYPE type) throws InvalidEvaluation {
        ...
        switch (type){
            ...
            case NO_CRIMINAL_RECORD_AND_MORE_EXPERIENCE: return x->!x.hasCriminalRecord()&&x.getEmploymentYears()>=10;
            case MORE_EXPERIENCE_AND_MORE_CREDIT_SCORE: return x->x.getEmploymentYears()>=10 && x.getCreditScore()>=500;
            case NO_CRIMINAL_RECORD_AND_MORE_CREDIT_SCORE: return x->!x.hasCriminalRecord()&&x.getCreditScore()>=500;
