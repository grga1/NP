...
// НЕ е public класата, за да може да стои во ист фајл со QueueTest
...
// Едноставен јазел за еднострано поврзана листа
class Node<T> {
...
// Ред имплементиран со еднострано поврзана листа + покажувачи кон глава/опаш
class Queue<T> {
    private Node<T> head;   // вадење (dequeue) од тука
    private Node<T> tail;   // додавање (enqueue) тука
    private int size;
    ...
    public T dequeue() throws EmptyQueueException {
        ...
        size--;
        if (size == 0) {
            // ако извадиме последен елемент, и tail мора да стане null
            tail = null;
        }
        return elem;
    }

    // Прв елемент без вадење (O(1))
    ...
    // Последен елемент без вадење (O(1))
